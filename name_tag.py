...
parser = argparse.ArgumentParser(
    description="Make your own name tag for the Feminist Python meetup!"
)
parser.add_argument(
    "--template",
    type=str,
    help="name of the svg file containing your template (in the templates/ directory)",
    required=True,
)
parser.add_argument(
    "--colors",
    help='file that defines the mapping between the colors and pronouns. Should be a json file with the following form {"pronoun":"hex-code"}',
    nargs="?",
...
parser.add_argument(
    "--defaultcolor",
    type=str,
    help="the hex-code of the color that is used if no mapping for pronoun is defined in the mapping file",
    nargs="?",
    default="009e73",
)
parser.add_argument(
    "--name", type=str, help="your name", nargs="?", default="Barbara"
)  # like Barbara Liskov as in the Liskov substitution principle
parser.add_argument(
    "--pronouns",
    type=str,
    help="your pronouns",
...
parser.add_argument(
    "--output",
    type=str,
    help="path to the output file for your rendered name tag",
    nargs="?",
    default="rendered/output.svg",
)
parser.add_argument(
    "--fingerprint",
    help="Encryption Key Fingerprint",
    nargs="?",
    type=check_hexadecimal,
)
parser.add_argument(
    "--gradient",
    help="flag for dynamically created gradient background",
    action="store_true",
)
...
background = background_tag["style"].replace(
    "fill:#ffdd55", "fill:#{}".format(color_map.get(pronouns, default_color))
)
